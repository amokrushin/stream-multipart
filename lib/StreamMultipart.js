...
class Multipart extends Duplex {
    ...
    _onHeader(part, prevPart, header) {
        ...
        if (header['content-type']) {
            ...
            if (parsed[0]) {
                metadata.contentType = parsed[0].toLowerCase();
                for (let i = 0; i < parsed.length; ++i) {
                    if (RE_CHARSET.test(parsed[i][0])) {
                        metadata.charset = parsed[i][1].toLowerCase();
                        break;
                    }
                }
            }
        ...
        if (header['content-disposition']) {
            ...
            for (let i = 0; i < parsed.length; ++i) {
                if (RE_NAME.test(parsed[i][0])) {
                    metadata.fieldname = parsed[i][1];
                } else if (RE_FILENAME.test(parsed[i][0])) {
                    metadata.filename = parsed[i][1];
        ...
        if (header['content-transfer-encoding']) {
            metadata.encoding = header['content-transfer-encoding'][0].toLowerCase();
        }

        if (metadata.contentType === 'application/octet-stream' || metadata.filename !== undefined) {
            this._onFile(part, prevPart, metadata);
        } else {
            this._onField(part, prevPart, metadata);
        }
    }

    _onFile(part, prevPart, metadata) {
        debug('gray', 'FILE START', metadata.filename);
        ...
        file.once('end', () => {
            debug('gray', 'FILE END', metadata.filename);
            this._state.counter.inProgress--;
            this._readNext();
            this._checkEnded();
            process.nextTick(() => {
    ...
    _onField(field, prevPart, metadata) {
        let fieldSize = 0;
        ...
        field.once('end', () => {
            this._waitForPartEnd(prevPart, () => {
                this.emit('field', buffer, truncated ? Object.assign({ truncated }, metadata) : metadata);
            });
            this._state.counter.inProgress--;
            this._checkEnded();
        });
