...
class Multipart extends Duplex {
    constructor(options) {
        ...
        const { defCharset, preservePath, highWaterMark, headers, asyncError } = options;
    ...
    _write(chunk, encoding, cb) {
        ...
        if (!this._state.needDrain && !this._state._pause) {
            cb();
        } else {
    ...
    _onFile(part, prevPart, { fieldname, filename, encoding, contentType, charset }) {
        ...
        const file = new FileStream(Object.assign({
            read: () => {
                if (!this._state._pause) {
                    return;
                }
                this._state._pause = false;
                this._readNext();
            },
        ...
        part.on('data', (data) => {
            ...
            if (fileSize > this.options.fileSizeLimit) {
                ...
                this._state._pause = !file.push(data);
            }
        });
    ...
    _checkEnded() {
        if (this._state.counter.inProgress === 0 && this._state.finished) {
            this.push(null);
        }
    }
